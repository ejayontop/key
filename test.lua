local Config = {
    ...
    UseManualKey      = true,
    ManualKeyURL      = "https://raw.githubusercontent.com/ejayontop/key/refs/heads/main/key.txt",
    ...
    HubDescription  = "Join on Discord for Free 3 Days Key" -- The text shown below the title
...
local function isManualKeyValid(key)
    -- This fetches the raw content of your GitHub file
    local success, response = safeRequest({Url = Config.ManualKeyURL, Method = "GET"})
    
    if success and response.StatusCode == 200 then
        -- We check if the exact key provided by the user exists in the file text
        -- We use string.find to look for the user's input within the fetched text
        return string.find(response.Body, key, 1, true) ~= nil
    end
    return false
end
...
local function redeemKey(key)
    -- Check Manual Key first if enabled
    if Config.UseManualKey then
        if isManualKeyValid(key) then
            if writefile then writefile(Config.KeyFileName, key) end
            return true, "Success (Manual)"
        end
    end

    -- Fallback to PlatoBoost
